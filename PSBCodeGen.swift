...
class PSBCodeGen: SessionDelegate {
    ...
    public func request<T: Decodable>(_ url: String, method: HTTPMethod, encoding: ParameterEncoding? = nil, errorCB: ((Error) -> Void)? = nil, successCB: Any, type: T.Type) {
        ...
        manager?.request(urlHost + url, method: method, headers: defaultHeaders).responseJSON { response in
            do {
                guard
                    ...
                    let successCB = successCB as? ((T) -> Void)
                else {
                ...
                if response.response?.statusCode == 401 {
                    NotificationCenter.default.post(name: NSNotification.Name.init(rawValue: "ResponseRecieveHTTPStatusCodeNotAuthorized"), object: nil)
                }
